...
@section('title')
    Edit Category | RESERVASI HOTEL Admin
@endsection
...
@section('content')
    ...
    <div class="form-login">
        <form action="{{ url('/category/update/' . $category->id_categories) }}" method="post" enctype="multipart/form-data">
            ...
            <textarea class="input" name="deskripsi" id="description" placeholder="Description">{{ old('deskripsi', $category->deskripsi) }}</textarea>
            @error('deskripsi')
                <p style="font-size: 10px; color: red">{{ $message }}</p>
            @enderror
